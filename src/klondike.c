...
static int init_vars(GameInfo* g)
{
    ...
    char title[40];
    char *items[3] =
    {"normal (cards dealt 3 at a time)","easy (cards dealt 1 at a time)","return to main menu"};

    snprintf(title,40,"Welcome to %s",names[g->game]);

    g->variation=menu(title,num_items,items,"Choose normal or easy:",0,items," ",0,items);
    switch(g->variation) 
    {
	case 0:	
	    g->turn_no=3;
