...
/* This function just writes out paragraphs of text to the main board
 * It handles flowing text over to a new page (primitively)
 * text: contains the text, can contain line breaks.
 * finish:  set to -1 for the first paragraph (clears board etc)
 *	    set to 0 for normal paragraph
 *	    set to 1 for last paragraph (displays "press any key")
 *	    set to 2 if both first and last paragraph.
 */ 
void 
wp(char* text,GameInfo* g)
{
    int y,maxy,maxx;
    int page_width;
    char *start_p,*end_p,*final_p;
    ...
    page_width = CARD_WIDTH*g->num_cols;
    getmaxyx(g->main,maxy,maxx);
	...
	if (y>maxy-BOARD_Y-2*BORDER-1 || (end_p>=final_p))
	{
	    getmaxyx(g->main,maxy,maxx);
    	    mvwprintw(g->main,maxy-BORDER-1,maxx-16,"press a key...");
	    box(g->main,0,0);
	    wrefresh(g->main);
	    wgetch(g->main);
	    wclear(g->main);
    ...
    /* Now return the cards to how they were before */
    init_board(g->main,g);
}

void 
write_text(FILE* f,GameInfo* g)
{
    int y,maxy,maxx;
    int page_width;
    int curr_len,add_space,prev_add_space,last_pos;
    char *start_p,*end_p,*final_p;
    char text[100]; /* this will contain each line we get from file */
    char dest[100]; /* this will contain each line before we print it */
	...
	else
	{
	    ...
	    while (end_p < final_p)
	    {
		end_p = start_p + (page_width-last_pos);
		if (start_p + (page_width-last_pos) >= final_p)
    		    end_p=final_p;
		else
	    	    /* search for the space closest to the end of current line*/
    		    while (*end_p != ' ' && end_p>start_p) end_p--;
